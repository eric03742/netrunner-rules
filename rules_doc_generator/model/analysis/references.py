...
def _construct_reference_map(
  ...
  match element:
    ...
    case Chapter(id=id, sections=sections, text=text):
      ...
      ref_map[id] = RefInfo(f'{chapter_index}', '章', text, id, toc=True)
    ...
    case Section(id=id, section_elements=section_elements, text=text, steps=steps):
      check_id_defined(ref_map, id)
      ref_map[id] = RefInfo(f'{chapter_index}.{section_index}', '节', element.toc_text(), id, toc=True)
      sub_ref_type = '步' if steps else '条'
    ...
    case Rule(id=id):
      if id is not None:
        check_id_defined(ref_map, id)
        ref_type = ref_type if ref_type else '条'
